...
** Last update Sun Mar 27 15:35:28 2016 Antoine Roig
*/
			     ...
			     char *prog)
{
  ...
  while (i < head.prog_size + 1)
    {
      if (board[0][info->a % MEM_SIZE] != 0)
      	{
      	  my_putstr("Not enough space\n");
      	  exit(0);
      	}
      board[1][info->a % MEM_SIZE] = id;
      board[0][info->a++ % MEM_SIZE] = prog[i++];
    }
  id++;
}
			     ...
			     unsigned char board[2][MEM_SIZE])
{
  ...
  champ_info(new_elem, board[0], info, head);
